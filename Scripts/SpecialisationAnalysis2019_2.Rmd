...
# By Default frame2webs sorts species alphabetically
## this finds the correct ordering 
FW%>% 
  arrange(Plant_species19) %>%
  distinct(Plant_species19, .keep_all = TRUE) %>%
  as.data.frame()%>%
  select(TaxoOrder) %>% 
  as.vector %>% 
  order(decreasing = TRUE) -> TaxoOrdering

 FW%>%
   distinct(Plant_species19, .keep_all = TRUE) %>%
arrange(desc(TaxoOrder))%>%
   select(Plant_species19,Plant_Family) -> SpFam

FW$PlantFamily %>%table %>%length -> NUMFAMILIES
FAMS<-factor(SpFam$Plant_Family,levels =  unique(SpFam$Plant_Family))



#SpFam$FAMCOLS<- rep(c('darkolivegreen', 'forestgreen'),ceiling(NUMFAMILIES/2)) [FAMS]  # Can use a nicer palette in future


SpFam$FAMCOLS<- rep(c('black', 'black'),ceiling(NUMFAMILIES/2)) [FAMS]  # Can use a nicer palette in future
...
IntMatrix<- frame2webs(ForPlot,
                       varnames=c('NicePlantName','Insect', 'Site', 'KilledPerM2'))

TaxoOrderWeb<-IntMatrix$BCI[TaxoOrdering,]

Insects<-colnames(TaxoOrderWeb)

colnames(TaxoOrderWeb)<- purrr::map_chr(as.list(Insects),
                                        function(str){
                                          Parts<- str_split(str, ' ')
...
par(mai=c(8,0,2,0), oma=c(3,0,3,0))
OneSideCCA(web = TaxoOrderWeb,
           text.rot = 90, labsize = 0.7 , 
           FAMILYNAMES =FAMS, FAMLABELPOS = 0.15)
dev.off()
...
## Network Ordered by specialisation

```{r}

Ordering<-  bipartiteD3::OrderByCrossover(TaxoOrderWeb)

TaxoOrderWebSort <- TaxoOrderWeb[rev(Ordering$PrimaryOrder), rev(Ordering$SecondaryOrder)]
par(mai=c(8,0,2,0), oma=c(3,0,3,0), xpd=TRUE)


png('../Figures/FoodwebLabelled.png', units = 'in',width = 15, height = 10, res = 1000)
par(mai=c(8,0,2,0), oma=c(3,0,3,0), xpd=TRUE)
plotweb(TaxoOrderWebSort,method = 'normal', text.rot = 90, labsize = 0.5, ybig = 2)
dev.off()

pdf('../Figures/FoodwebLabelled.pdf',units = 'in', width = 15, height = 10)
par(mai=c(8,0,2,0), oma=c(3,0,3,0), xpd=TRUE)
plotweb(TaxoOrderWebSort,method = 'normal', text.rot = 90, labsize = 0.5, ybig = 2)
dev.off()


png('../Figures/Foodweb.png', units = 'in',width = 15, height = 10, res = 1000)
par(mai=c(8,0,2,0), oma=c(3,0,3,0), xpd=TRUE)
plotweb(TaxoOrderWebSort,method = 'normal', text.rot = 90, labsize = 0.5, ybig = 2, text.high.col = 'white', text.low.col = 'white')
dev.off()
...
```{r}
pdf('../Figures/FigureS4.pdf', width = 10, height = 18)

par(mfrow=c(2,1))
par(mai=c(8,2,6,2))

ForPlot %>%
  mutate(SubWeb = ifelse(Order=='Lepidoptera',
                         'Lepidoptera',
                         'Coleoptera'))-> ForPlot

for(i in 1:2){
  
  SUBWEB<-c('Coleoptera','Lepidoptera')[i]
  
  ForPlot %>%
    filter(SubWeb==SUBWEB)%>%
    as.data.frame %>%
    frame2webs(varnames=c('NicePlantName','Insect',
                          'Site', 'KilledPerM2')) -> SpIntMatrix
  ...
  ForPlot%>% 
    ...
    select(TaxoOrder) %>% 
    as.vector %>% 
    order(decreasing = TRUE) -> SpTaxoOrdering
  
  OneSideCCA(SpIntMatrix$BCI[SpTaxoOrdering,],
             col.low = 1,
             bor.col.low = 1, 
             text.rot = 90,
             text.low.col = 1,
             labsize = 0.7, FAMILYNAMES = NULL)
...
png('../Figures/FigureS4.png', width = 10, height = 18, units='in', res=1000)

par(mfrow=c(2,1))
par(mai=c(8,2,6,2))

ForPlot %>%
  mutate(SubWeb = ifelse(SubWeb=='Lepidoptera',
                         'Lepidoptera',
                         'Coleoptera'))-> ForPlot
...
# Plotting Again Without Singletons
...
NoSingFW$PlantFamily %>%table %>%length -> NUMFAMILIES
FAMS<-factor(SpFam$Plant_Family,levels =  unique(SpFam$Plant_Family))
...
pdf('../Figures/FigureS5.pdf', width = 12, height = 12)
par(mai=c(8,0,2,0), oma=c(3,0,3,0))
OneSideCCA(web = TaxoOrderWeb,
           text.rot = 90, labsize = 0.7 , 
           FAMILYNAMES =FAMS, FAMLABELPOS = 0.15)
...
png('../Figures/FigureS5.png', units = 'in',width = 12, height = 12, res = 1000)
par(mai=c(8,0,2,0), oma=c(3,0,3,0))
OneSideCCA(web = TaxoOrderWeb,
           text.rot = 90, labsize = 0.7 , 
           FAMILYNAMES =FAMS, FAMLABELPOS = 0.15)
...
```{r}
pdf('../Figures/FigureS5b.pdf', width = 10, height = 12)
...
png('../Figures/FigureS5b.png', width = 10, height = 12, units='in', res=600)
